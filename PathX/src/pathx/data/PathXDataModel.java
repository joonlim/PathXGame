...
import static pathx.ui.PathXMiniGame.checkedSound;
import static pathx.ui.PathXMiniGame.isEmptyGasEnabled;
import static pathx.ui.PathXMiniGame.isFlattenTireEnabled;
import static pathx.ui.PathXMiniGame.isFlyEnabled;
import static pathx.ui.PathXMiniGame.isGameStarted;
import static pathx.ui.PathXMiniGame.isIntagibilityEnabled;
import static pathx.ui.PathXMiniGame.isInvincibilityEnabled;
import static pathx.ui.PathXMiniGame.isMindlessTerrorEnabled;
import static pathx.ui.PathXMiniGame.isStealEnabled;
...
//        System.out.println("Y INCREASE " + PathXPanel.sourceY1);
      ...
      public void movePlayer(Intersection i1, Intersection i2)
    {
        ...
        if (isFlyEnabled) {
            ...
            path = gameGraph.findShortestPathToHome(i1, i2);
        }
...
//            location = level.getStartingLocation();
      ...
      //  System.out.println("isGameStarted: " + PathXMiniGame.isGameStarted);
       // System.out.println("NOT isMovingToTarget: " + !playerCar.get(0).isMovingToTarget());
        ...
        System.out.println("isFlattenTireEnabled: " + isFlattenTireEnabled);
        System.out.println("isEmptyGasEnabled: " + isEmptyGasEnabled);
         System.out.println("isMindlessTerrorEnabled: " + isMindlessTerrorEnabled);
        
         //FOR MINDLESS TERROR
        if(isMindlessTerrorEnabled) {
            
             for(BanditCar b: movingBandits) {
            if(errorX - margin <= b.getX() && errorX + margin >= b.getX()
               && errorY - margin <= b.getY() && errorY + margin >= b.getY()     ) {
                System.out.println("MINDLESS TERROR BANDIT!");
                b.invokeMindlessTerror();
                if(isMindlessTerrorEnabled)
                isMindlessTerrorEnabled = false;
            }
        }
         for(ZombieCar z: movingZombies) {
            if(errorX - margin <= z.getX() && errorX + margin >= z.getX()
               && errorY - margin <= z.getY() && errorY + margin >= z.getY()     ) {
                System.out.println("MINDLESS TERROR ZOMBIE!");
                z.invokeMindlessTerror();
                if(isMindlessTerrorEnabled)
                isMindlessTerrorEnabled = false;
            }
        }
          for(PoliceCar p: movingPolice) {
            if(errorX - margin <= p.getX() && errorX + margin >= p.getX()
               && errorY - margin <= p.getY() && errorY + margin >= p.getY()     ) {
                System.out.println("MINDLESS TERROR POLICE!");
                p.invokeMindlessTerror();
                if(isMindlessTerrorEnabled)
                isMindlessTerrorEnabled = false;
            }
          }
        ...
        //FOR STOPPING AN ENEMY CAR
        if(isFlattenTireEnabled || isEmptyGasEnabled) {
        
        for(BanditCar b: movingBandits) {
            if(errorX - margin <= b.getX() && errorX + margin >= b.getX()
               && errorY - margin <= b.getY() && errorY + margin >= b.getY()     ) {
                System.out.println("CAUGHT A BANDIT!");
                b.setCarStopped(true);
                if(isFlattenTireEnabled)
                isFlattenTireEnabled = false;
                else isEmptyGasEnabled = false;
            }
        }
         for(ZombieCar z: movingZombies) {
            if(errorX - margin <= z.getX() && errorX + margin >= z.getX()
               && errorY - margin <= z.getY() && errorY + margin >= z.getY()     ) {
                System.out.println("CAUGHT A ZOMBIE!");
                z.setCarStopped(true);
                if(isFlattenTireEnabled)
                isFlattenTireEnabled = false;
                else isEmptyGasEnabled = false;
            }
          ...
          for(PoliceCar p: movingPolice) {
            if(errorX - margin <= p.getX() && errorX + margin >= p.getX()
               && errorY - margin <= p.getY() && errorY + margin >= p.getY()     ) {
                System.out.println("CAUGHT A POLICE!");
                p.setCarStopped(true);
                if(isFlattenTireEnabled)
                isFlattenTireEnabled = false;
                else isEmptyGasEnabled = false;
            }
    ...
    public void updateAll(MiniGame game)
    {
        try
        {
            ...
            for (int i = 0; i < movingPolice.size(); i++)
            {
                ...
                if (!movingPlayerCar.isEmpty() && !isIntagibilityEnabled) { // && !isInvincibilityEnabled) {

                    if ((tile.getX() + 2.5 >= movingPlayerCar.get(0).getX() && tile.getX() - 2.5 <= movingPlayerCar.get(0).getX())
                            && (tile.getY() + 2.5 >= movingPlayerCar.get(0).getY() && tile.getY() - 2.5 <= movingPlayerCar.get(0).getY())) {
                        if(isStealEnabled) {
                            levelBounty += levelBounty*.2;
                        }
                        else if(isInvincibilityEnabled) {
                            tile.setCarStopped(true);
                        }
                        else  {
            ...
            for (int i = 0; i < movingZombies.size(); i++)
            {
                ...
                if (!movingPlayerCar.isEmpty() && !isIntagibilityEnabled) {
